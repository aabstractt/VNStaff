...
public class FreezeListener implements Listener {
    ...
    public void onHit(EntityDamageEvent e){
        ...
        Session playerSession = SessionManager.getInstance().getSession(e.getEntity().getUniqueId());
        if(playerSession.isFreezed()) e.setCancelled(true);
    }
    @EventHandler(priority = EventPriority.NORMAL)
    public void onPlayerAttack(EntityDamageByEntityEvent e){
        if(e.getDamager() == null || !(e.getDamager() instanceof Player)) return;
        Session playerSession = SessionManager.getInstance().getSession(e.getDamager().getUniqueId());
        if(playerSession.isFreezed()) e.setCancelled(true);
    }
    @EventHandler(priority = EventPriority.NORMAL)
    public void onMove(PlayerMoveEvent e) {
        ...
        Session playerSession = SessionManager.getInstance().getSession(e.getPlayer().getUniqueId());
